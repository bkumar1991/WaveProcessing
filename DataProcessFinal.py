...
def perform_matrix_inversion(trace_offsets, peak_envelopes, degree=1, lambda_reg=1e-4):
...
def perform_matrix_inversion_for_peak_time(trace_offsets, peak_time, degree=1, lambda_reg=1e-4):
...
def save_plots_with_optimized_inversion(folder, save_path1, save_path2, save_path3, save_path4, save_path6, save_path7, save_path8, save_path9,save_path10 ):
    ...
    # Perform matrix inversion to get the initial guess for optimization
    lambda_reg = 1e-4 
    m, G = perform_matrix_inversion(trace_offsets, peak_envelopes, degree=1, lambda_reg= lambda_reg)
    
    if m is None:
        print("Error in performing matrix inversion. Exiting function.")
        return
    m1, G1 = perform_matrix_inversion_for_peak_time(trace_offsets, peak_times, degree=1, lambda_reg= lambda_reg)
    ...
    with open(output_txt_file, 'w') as f1:
        ...
        for i in range(min_len):
            ...
            neg_streak = 0           #  positive correlation → NORMAL CASE
