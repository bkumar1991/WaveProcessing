...
fw = open(catalog + '.output', 'a')  #change 'w' to 'a' if want to append to the existing file
...
# loop over traces
            ...
            #for l in range(5):    
                ...
                #trflt.write(Datapath + stcode + str(freq[k]), format = 'SAC')
                #tr.plot(type = 'section', starttime = otime, orientation = 'horizontal')
                #trflt.plot(type = 'section', starttime = otime, orientation = 'horizontal')
...
##                plt.plot(tevlp, trflt0.data, "k")
##                plt.plot(tevlp, evlp, 'k:')
##                plt.vlines(tmin1, 0, Amax/2, "r")
##                plt.vlines(tmin2, 0, Amax/2, "r")
##                plt.plot(tmax,Amax,"ro")
##                plt.plot(tph,0,"bo")
##                stitle = 'f = ' + str(float("{:.4f}".format((freqmin+freqmax)/2.0))) + \
##                              '   Amax/Aavg = ' + str(float("{:.1f}".format(Amax/Aavg))) \
##                               + '    Tmax-Tpre = ' + str(float("{:.1f}".format(tmax-tph)))
##                plt.title (stitle)
##                plt.show()
...
## these can be changed based on your own tests on events with different magnitudes and at different distances
            ...
            if len(stXfltkeep) != 0:
                ...
                stXfltnew.plot(type = 'section', time_down = True, scale = 2, outfile = Fltpath + '/f-' + str(freq[k]) + '.png')
                stXfltkeep.plot(type = 'section', time_down = True, scale = 2, outfile = Fltpath + '/f-' + str(freq[k]) + 'kp.png')
